...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($_POST['phone_number']) && isset($_POST['student_id']) && isset($_POST['institute']) && isset($_POST['emergency_contact']) && isset($_POST['emg_cont_name'])) {
        ...
        try {
            ...
            $stmt = $conn->prepare("INSERT INTO student_details (phone_number, student_id, institute, emergency_contact, emg_cont_name) VALUES (?, ?, ?, ?, ?)");
            ...
            $stmt->bind_param("sssss", $phone_number, $student_id, $institute, $emergency_contact, $emg_cont_name);
            
            // Execute the prepared statement
            if ($stmt->execute()) {
                // If the insert was successful, set a success message
                $_SESSION['success_message'] = "Student details confirmed successfully.";
            } else {
